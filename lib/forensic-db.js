...
const SCHEMA_STATEMENTS = [
    `CREATE TABLE IF NOT EXISTS projects (
        ...
        organization TEXT,
        source_type TEXT NOT NULL DEFAULT 'browser-upload',
...
function initializeDatabase(database) {
    ...
    ensureColumn(database, 'projects', 'source_type', "TEXT NOT NULL DEFAULT 'browser-upload'");
    ...
    if (projectColumns.some(column => column.name === 'source_location')) {
        database.exec('ALTER TABLE projects DROP COLUMN source_location');
    }
...
function getOrCreateProjectId(database, project = DEFAULT_PROJECT) {
    ...
    database
        .prepare(
            `INSERT INTO projects (project, source_type, created_at, updated_at)
...
function createProject(database, { project, organization = null }) {
        ...
        const result = database
            .prepare(
                `INSERT INTO projects (
                    project, organization, source_type, created_at, updated_at
                ) VALUES (?, ?, 'browser-upload', ?, ?)`,
            )
            .run(
                normalizedProject,
...
function deleteProject(database, projectId) {
    return withTransaction(database, () => {
        ...
        if (findActiveProjectJob(database, projectId)) {
            const error = new Error('Ein laufendes Projekt kann nicht gelöscht werden.');
            error.status = 409;
            throw error;
        }
