...
function getOrCreateProjectId(database, project = DEFAULT_PROJECT) {
    ...
    database
        .prepare('INSERT INTO projects (project, created_at) VALUES (?, ?) ON CONFLICT(project) DO NOTHING')
...
function migrateLegacySnapshot(database, project, snapshot) {
    return withTransaction(database, () => {
        const existing = Number(database.prepare('SELECT COUNT(*) AS count FROM documents').get().count);

        if (existing > 0) {
            throw new Error('Forensic database already contains documents; migration was not applied');
        }

        const projectId = getOrCreateProjectId(database, project);
        const problemDocuments = Array.isArray(snapshot.problemDocuments) ? snapshot.problemDocuments : [];
        const scanProgress = snapshot.scanProgress && typeof snapshot.scanProgress === 'object' ? snapshot.scanProgress : {};
        const completed = scanProgress.completed && typeof scanProgress.completed === 'object' ? scanProgress.completed : {};
        const errors = scanProgress.errors && typeof scanProgress.errors === 'object' ? scanProgress.errors : {};
        const importedFiles = new Set();

        for (const document of problemDocuments) {
            if (!document || typeof document !== 'object' || !textOrNull(document.file)) continue;

            writeCompletedScan(
                database,
                projectId,
                document,
                textOrNull(completed[document.file]) || textOrNull(document.scanned_at) || nowIso(),
            );
            importedFiles.add(document.file);
        }

        for (const [file, completedAt] of Object.entries(completed)) {
            ...
            writeCompletedScan(
                ...
                {
                    file,
                    document_id: path.basename(file, path.extname(file)),
                    problems: [],
                    scanned_at: textOrNull(completedAt),
                    note: 'Imported from redaction_scan_progress.json.',
                },
                textOrNull(completedAt) || nowIso(),
            );
            importedFiles.add(file);
        }

        for (const [file, value] of Object.entries(errors)) {
            const timestamp = textOrNull(value?.at) || nowIso();
            const documentPk = upsertDocument(
                database,
                projectId,
                {
                    file,
            ...
            database
                ...
                .run(documentPk, String(value?.error || value || 'Unknown scan error'), timestamp, timestamp);
        }

        const humanProgress =
            snapshot.humanProgress && typeof snapshot.humanProgress === 'object' ? snapshot.humanProgress : {};
        const reviewed = humanProgress.reviewed && typeof humanProgress.reviewed === 'object' ? humanProgress.reviewed : {};
        const acceptedRecords = Array.isArray(snapshot.acceptedProblems) ? snapshot.acceptedProblems : [];
        const acceptedById = new Map(
            acceptedRecords
                .filter(record => textOrNull(record?.problem_id))
                .map(record => [record.problem_id, record]),
        );
        const decisions = new Map();

        for (const [problemId, decision] of Object.entries(reviewed)) {
            if (decision === 'accepted' || decision === 'skipped') decisions.set(problemId, decision);
        }

        for (const record of acceptedRecords) {
            if (textOrNull(record?.problem_id)) decisions.set(record.problem_id, 'accepted');
        }
        ...
        for (const [problemId, decision] of decisions) {
            const finding = database
                .prepare(
                    `SELECT finding.id
                     FROM findings finding
                     JOIN documents document ON document.id = finding.document_pk
                     WHERE document.project_id = ? AND finding.problem_id = ?`,
                )
                .get(projectId, problemId);
            ...
            const reviewedAt =
                textOrNull(acceptedById.get(problemId)?.reviewed_at) || textOrNull(humanProgress.updated_at) || nowIso();
            upsertReview.run(finding.id, decision, reviewedAt, reviewedAt);
        }

        const runTimestamp = textOrNull(scanProgress.updated_at) || nowIso();
        database
            .prepare(
                `INSERT INTO scan_runs (
                    project_id, kind, status, selected_count, processed_count, skipped_count,
                    problem_documents_total, started_at, updated_at, completed_at
                ) VALUES (?, 'legacy-json-import', 'imported', ?, ?, ?, ?, ?, ?, ?)`,
            )
            .run(
                projectId,
                Number(scanProgress.selected || Object.keys(completed).length + Object.keys(errors).length),
                Number(scanProgress.processed_this_run || 0),
                Number(scanProgress.skipped_this_run || 0),
                Number(scanProgress.problem_documents_total || problemDocuments.length),
                runTimestamp,
                runTimestamp,
                runTimestamp,
            );
        database
            .prepare(
                `INSERT INTO forensic_metadata (key, value, updated_at)
        ...
        return {
            project,
            documents: Number(
                database.prepare('SELECT COUNT(*) AS count FROM documents WHERE project_id = ?').get(projectId).count,
            ),
            findings: Number(
                database
                    .prepare(
                ...
                database
                    .prepare(
                        `SELECT COUNT(*) AS count
                         ...
                         JOIN findings finding ON finding.id = review.finding_pk
                         JOIN documents document ON document.id = finding.document_pk
                         WHERE document.project_id = ? AND review.decision = 'accepted'`,
                    )
                    .get(projectId).count,
            ),
            missingReviews,
